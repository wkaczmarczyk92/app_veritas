...
<template>
    <GuestLayout>
        ...
        <form @submit.prevent="submit">
            ...
            <div class="mt-4">
                <TInput type="password" label="Hasło" :error="form.errors.password ? form.errors.password : ''"
                    v-model:model_value="form.password" :clearable="false" />
            </div>
            ...
            <div class="tw-flex tw-flex-col tw-gap-2 tw-justify-end tw-text-right tw-place-items-end tw-mt-4">
                ...
                <Button type="submit" value="Zaloguj się" variant="tonal" color="blue" :disabled="form.processing" />
            </div>
                    ...
                    <div v-if="password_reset_view == null">
                        ...
                        <div class="tw-flex tw-flex-col tw-mt-4">
                            ...
                            <div class="tw-text-blue-600 hover:tw-cursor-pointer hover:tw-text-blue-600 hover:tw-underline"
                                @click="pesel_error = ''; password_reset_view = 'by_sms'">Użyj jednorazowego hasła SMS
                                do
                                zmiany hasła</div>
                        </div>
                    </div>
                    <div v-else-if="password_reset_view != null"
                        class="tw-mt-10 tw-bg-white tw-p-6 tw-rounded-xl tw-shadow-lg tw-shadow-gray-500">
                        <Transition name="slide-fade" mode="out-in">

                            <div v-if="password_reset_view == 'by_phone'" class="tw-px-2 md:tw-px-10">
                                <PasswordWithHelp :form="new_password_form"
                                    @new-password-request="new_password_request()" @change-view="change_view()">
                                </PasswordWithHelp>
                            </div>

                            <div v-else-if="password_reset_view == 'by_sms'" class="tw-px-0 md:tw-px-10">
                                <h3
                                    class="tw-font-semibold tw-text-md md:tw-text-lg tw-leading-tight tw-text-left tw-mt-6">
                                ...
                                <div class="tw-flex tw-flex-col">
                                    <Transition name="slide-fade" mode="out-in">
                                        <div class="tw-text-left"
                                            ...
                                            <SMSPasswordForm :form="new_password_form"
                                                @submit-sms-code="submit_sms_code()" @use-code="use_code()"
                                                @change-view="change_view()"></SMSPasswordForm>
                                        </div>
                                    ...
                                    <div class="tw-text-red-600 hover:tw-underline hover:tw-text-red-800 hover:tw-cursor-pointer tw-mt-6"
                                        @click="pesel_error = ''; password_reset_view = 'by_phone'">Mam problem ze
                                        zmianą
                                        hasła przez kod SMS
                                    </div>
