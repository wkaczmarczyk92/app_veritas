...
payout_requests.value = await usePayoutRequestStore.loadRequestsData({ page_name: 'for_approval_page', route_name: 'load.for.approval.payout.requests' });

async function reloadRequestsByURL(url) {
    payout_requests.value = await usePayoutRequestStore.loadRequestsData({ page_name: 'for_approval_page', full_route: url });
}

async function reloadRequestsByPageNumber(page) {
    payout_requests.value = await usePayoutRequestStore.loadRequestsData({ page: page, page_name: 'for_approval_page', route_name: 'load.for.approval.payout.requests' });
}
...
async function move_to_in_progress() {
    spinner_visible.value = true;
    disabled.value = true;
    await axios.patch(route('payoutrequests.update'), { to_update: payout_reuqests_for_action.value, status: 'in_progress' })
        .then(response => {
            if (response.data.success) {
                useAlertStore.pushAlert('success', 'Wybrane wnioski o wypłatę zostały przeniesione do Oczekujących.');
            } else {
...
<template>
    <div v-if="payout_requests && payout_requests.data.length <= 0">
        <StaticInfoAlert class="tw-mt-10">Brak wnoisków do akceptacji.</StaticInfoAlert>
    </div>
    <div v-else>
        ...
        <div class="tw-bg-gray-100 tw-shadow-xl tw-rounded tw-mb-6"
            ...
            <div class="tw-px-6 tw-pt-6 tw-text-center">
                <Header h="2" value="Wnioski do akceptacji" :center="true" icon="fa-solid fa-check tw-mr-2"
                    icon_color="tw-text-green-500"></Header>
            </div>
            <div class="tw-flex tw-flex-row tw-mt-6 tw-text-gray-800 tw-mb-3 tw-px-6 tw-pt-2">
                ...
                <div class="tw-ml-4 tw-underline tw-text-green-600 hover:tw-text-green-800 hover:tw-cursor-pointer"
                    @click="move_to_in_progress()" :class="disabled ? 'tw-pointer-events-none' : ''">
            ...
            <TableDefault :headers="headers">
                <tr class="hover:tw-bg-grey-lighter" v-for="(payout_request, index) in payout_requests.data">
                    ...
                    <td class="tw-py-4 tw-px-6 tw-border-b tw-border-grey-light">{{ payout_request.black_list_comment ??
                        'brak komentarza' }}
                    </td>
