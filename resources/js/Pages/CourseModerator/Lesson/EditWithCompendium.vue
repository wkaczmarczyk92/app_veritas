            ...
            <div class="tw-grid lg:tw-grid-cols-2 tw-grid-cols-1 tw-gap-4">
                <v-card :loading="form.progress" title="Edytuj lekcję" subtitle="podstawowe dane" class="!tw-p-4">
                    ...
                    <v-card-actions>
                        ...
                        <v-btn variant="outlined" color="#dc2626" size="large" :href="route('course_moderator.compendium.lesson.show', {
                            compendium_id: props.lesson.lessonable_id,
...
const breadcrumbs = [
    ...
    },
    {
        ...
        disabled: false,
        href: route('course_moderator.compendium.lesson.show', {
            compendium_id: props.lesson.lessonable.id,
            lesson_id: props.lesson.id
        })
    },
    {
...
const submit = async () => {
    ...
    if (response.success) {
        setTimeout(() => {
            router.visit(route('course_moderator.compendium.lesson.show', {
                compendium_id: props.lesson.lessonable.id,
                lesson_id: props.lesson.id
            }))
        }, 2000)
