<template>
    ...
    <CourseModeratorLayout>
        ...
        <Processing v-if="delete_compendium_progress" msg="Usuwanie kompendium wraz z zawartością..." />
        <MainContent>
            <div class="tw-grid lg:tw-grid-cols-2 tw-grid-cols-1 tw-gap-4">

                <v-card :color="'white'" class="tw-mb-4 tw-shadow-2xl !tw-px-4" :title="compendium.name"
                    :subtitle="'Utworzono: ' + format(compendium.created_at)">
                    <template v-slot:prepend>
                        <i class="fa-regular fa-memo-circle-info tw-text-2xl"></i>
                    </template>
                    <template v-slot:title>
                        <div class="tw-flex tw-flex-row tw-justify-between">
                            <div>{{ compendium.name }}</div>
                            <v-btn color="#0284c7" text="Podgląd" variant="outlined" :disabled="false" size="small"
                                :href="route('course_moderator.compendium.edit', {
                                    id: compendium.id
                                })">
                                <template v-slot:prepend>
                    ...
                    <template v-slot:subtitle>
                        <div class="tw-flex tw-flex-row tw-gap-2 tw-mt-2">
                            <v-chip variant="outlined">
                                Utworzono: {{ format(compendium.created_at) }}
                            </v-chip>
                            <v-chip v-if="compendium.lessons_count > 0" variant="outlined">
                                Ilość lekcji: {{ compendium.lessons_count }}
                            </v-chip>
                            <v-chip variant="outlined" :color="compendium.visibility.id == 1 ? '#ea580c' : '#16a34a'">
                                {{ compendium.visibility.name_pl }}
                            </v-chip>
                        </div>
                    </template>
                    <v-card-text class="tw-flex tw-gap-4 tw-flex-col tw-my-6">
                        <div v-if="compendium.roles.length > 0 || compendium.company_branches.length > 0">
                            <h2 class="tw-text-lg tw-mb-2">Widoczność</h2>
                            <div v-if="compendium.roles.length > 0" class="tw-flex tw-flex-row tw-gap-1">
                                <v-chip v-for="(role, index) in compendium.roles" variant="outlined" color="#ea580c">
                                    {{ role.name_pl }}
                                </v-chip>
                            </div>
                            <div v-if="compendium.company_branches.length > 0"
                                class="tw-flex tw-flex-row tw-gap-1 tw-mt-1">
                                <v-chip v-for="(branch, index) in compendium.company_branches" variant="outlined"
                                    color="#16a34a">
                                    {{ branch.name }}
                                </v-chip>
                            </div>
                        ...
                        <div v-html="compendium.description" class="tw-mt-6"></div>
                    </v-card-text>
                </v-card>
                    ...
                    <v-card-text class="tw-flex tw-gap-4 tw-flex-col tw-my-6">
                        <div>
                            <v-btn color="#22c55e" text="Podgląd" variant="outlined" :disabled="false"
                                :href="route('course_moderator.compendium.lesson.create', { compendium_id: compendium.id })"><template
                                    v-slot:prepend>
                            ...
                            <draggable v-model="lessons" group="people" @start="drag = true"
                                ...
                                <template #item="{ element }">
                                    <v-list-item :key="element.id"
                                        ...
                                        <template v-slot:title>
                                            <a class="tw-text-blue-700 hover:tw-text-blue-900 hover:tw-underline" :href="route('course_moderator.compendium.lesson.show', {
            ...
            <div class="tw-flex tw-flex-row-reverse">
                <v-btn color="#dc2626" text="Podgląd" size="small" variant="tonal" :disabled="false"
                    ...
                    Usuń kompendium wraz ze wszystkimi danymi
                </v-btn>
            </div>
        </MainContent>
    </CourseModeratorLayout>
...
console.log('role', props.compendium.roles)
console.log('company branches', props.compendium.company_branches)

const lessons = ref(props.compendium.lessons)
...
const delete_compendium = async () => {
    if (confirm('Na pewno chcesz usunąć kompendium wraz z całą zawartością?')) {
        ...
        let response = await axios.delete(route('course_moderator.compendium.destroy', props.compendium.id))
        response = response.data

        alert_store.pushAlert(response.alert_type, response.msg)

        if (response.success) {
            router.visit(route('course_moderator.compendium.index'))
        } else {
