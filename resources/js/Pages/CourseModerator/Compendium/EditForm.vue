            ...
            <div class="tw-grid lg:tw-grid-cols-2 tw-grid-cols-1 tw-gap-4">
                <v-card :loading="form.progress" title="Edytuj kompendium" subtitle="podstawowe dane" class="!tw-p-4">
                    ...
                    <v-card-text class="tw-flex tw-gap-4 tw-flex-col tw-my-6">
                        ...
                        <v-select :items="visibility" item-title="name_pl" item-value="id" label="Widoczność kompendium"
                        ...
                        <h2 class="tw-text-base">Kompendium dostępne dla</h2>
                    ...
                    <v-card-actions>
                        <v-btn variant="outlined" color="#0284c7" size="large" @click="submit()"
                            ...
                            Aktualizuj kompendium
                        </v-btn>
                        <v-btn variant="outlined" color="#dc2626" size="large" :href="route('course_moderator.compendium.show', {
                            id: compendium.id
                        })" :disabled="form.progress">
...
console.log('compendium', props.compendium)
...
const breadcrumbs = [
    ...
    {
        title: props.compendium.name ?? 'Błąd',
        disabled: false,
...
const form = ref({
    name: props.compendium.name,
    description: props.compendium.description,
    time_to_read: props.compendium.time_to_read,
    visibility_id: props.compendium.visibility_id,
    permissions: {
        roles: props.compendium.roles_id,
        company_branches: props.compendium.company_branches_id
    },
    progress: false
})

console.log('form edit', form.value)
